...
@section('title', 'View your status')
@section('content')
@php
    $requested = request()->boolean('sent') || session('participant_status_requested');
    $sentEmail = session('pf_status_email');
    $maskedEmail = null;
...
@if($requested)
    ...
    <div class="text-center">
        ...
        <p class="mx-auto mt-3 max-w-md text-[14px] leading-6 text-slate-600">
            @if($maskedEmail)
                We’ve sent a secure link to <span class="font-medium text-slate-800">{{ $maskedEmail }}</span>.
            @else
                We’ve sent a secure link to your email.
            @endif
            Open it to view your progress and certificate.
        </p>
    ...
    <div class="mt-8 rounded-xl border border-slate-200 bg-white p-5 text-center">
        ...
        <form class="mt-3" method="POST" action="{{ route('forms.public.status.access.request', $form->public_token) }}" data-resend-form>
            @csrf
            <input type="hidden" name="email" value="{{ $sentEmail }}">
            <button class="button-primary w-full sm:w-auto sm:min-w-56" data-resend-button disabled>
                <span data-resend-idle class="hidden">Resend the link</span>
        ...
        <p class="mt-4 text-[12px] text-slate-400">
            Wrong address? <a class="font-medium text-accent-600 hover:underline" href="{{ route('forms.public.status', $form->public_token) }}">Start over</a>.
        </p>
    </div>
    ...
    <script nonce="{{ $cspNonce }}">
        (function () {
            ...
            var tick = function () {
                remaining -= 1;
                if (remaining <= 0) { button.disabled = false; wait.classList.add('hidden'); idle.classList.remove('hidden'); return; }
            ...
            form.addEventListener('submit', function () {
                ...
                idle.classList.add('hidden'); wait.classList.add('hidden'); sending.classList.remove('hidden');
            });
        })();
    </script>
@else
    ...
    <header class="text-center">
        ...
        <h1 class="mt-1 text-[22px] font-bold leading-8 text-slate-900">View your participant status</h1>
        <p class="mx-auto mt-4 max-w-md text-[14px] leading-6 text-slate-600">
            Enter your email and we’ll send you a secure link to privately view your required steps and certificate.
        </p>
    </header>

    @if(session('participant_status_error'))
        <div class="mt-8 flex items-start gap-2.5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-[13px] leading-5 text-amber-900" role="alert">
            ...
            <span>That link was invalid or has expired. Enter your email below and we’ll send a fresh one.</span>
        </div>
    @endif
    ...
    <form class="mt-8" method="POST" action="{{ route('forms.public.status.access.request', $form->public_token) }}" data-access-form>
        @csrf
        <label class="block">
        ...
        <div class="mt-8 flex flex-col items-center gap-4">
            <button class="button-primary w-full sm:w-auto sm:min-w-56" data-access-button>
                <span data-access-label>Send me a link</span>
            ...
            <p class="max-w-md text-center text-[12px] leading-5 text-slate-400">The response is the same whether or not the address has a participant record.</p>
        </div>
    </form>
