...
@php
    ...
    $statusLabels = ['ready' => 'Ready to send', 'queued' => 'Queued', 'sending' => 'Sending email', 'sent' => 'Accepted by provider', 'failed' => 'Failed', 'missing_email' => 'Email needed'];
@endphp
        ...
        <div class="relative flex min-h-0 flex-1">
            ...
            <aside class="studio-side-panel studio-left-panel" data-panel="certificates">
                ...
                <div class="min-h-0 flex-1 overflow-y-auto p-2" data-certificate-list>
                    @foreach($participants as $participant)
                        @php $certificate = $participant->certificate_record; $status = $participant->cert_status; @endphp
                        <article class="studio-certificate-row" data-certificate-row="{{ $participant->public_id }}" data-search="{{ Str::lower($participant->full_name.' '.$participant->email.' '.$certificate?->verification_code) }}" data-status="{{ $status }}" data-name-update-url="{{ route('admin.participants.name', [$webinar, $participant]) }}">
                            ...
                            <button type="button" class="min-w-0 flex-1 text-left" data-preview-select="{{ $participant->public_id }}">
                                ...
                                <span class="mt-2 flex items-center justify-between pl-16"><span class="certificate-status certificate-status-{{ $status }}" data-status-badge>{{ $statusLabels[$status] ?? ucfirst($status) }}</span>@if($status === 'failed')<span class="text-[10px] font-semibold text-red-600">Retry available</span>@endif</span>
                            </button>
                        </article>
                    @endforeach
                </div>
